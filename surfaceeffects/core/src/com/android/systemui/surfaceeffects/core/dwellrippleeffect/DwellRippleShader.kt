...
import com.android.systemui.surfaceeffects.core.utils.MathUtils.lerp

/**
 * Shader class that renders a distorted ripple for the UDFPS dwell effect. Adjustable shader
 * parameters:
 * - progress
 * - origin
 * - color
 * - time
 * - maxRadius
 * - distortionStrength. See per field documentation for more details.
 *
 * Modeled after frameworks/base/graphics/java/android/graphics/drawable/RippleShader.java.
 */
class DwellRippleShader : RuntimeShader(SHADER) {
    companion object {
    ...
    var progress: Float = 0.0f
        set(value) {
            field = value
            setFloatUniform("in_radius", (1 - (1 - value) * (1 - value) * (1 - value)) * maxRadius)
            setFloatUniform("in_blur", lerp(1f, 0.7f, value))
        }
    ...
    fun applyConfig(dwellEffectConfig: DwellEffectConfig) {
        origin = Point(dwellEffectConfig.centerX.toInt(), dwellEffectConfig.centerY.toInt())
        maxRadius = dwellEffectConfig.maxRadius
        color = dwellEffectConfig.color
    }
}
